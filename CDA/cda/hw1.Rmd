...
male_sample <- strsplit(male_sample,',')[[1]]
male_sample <- gsub("\n","",male_sample)
female_sample <- strsplit(female_sample,',')[[1]]
female_sample <- gsub("\n","",female_sample)
countmale <- length(male_sample)
countfemale <- length(female_sample)
gender <- c(rep('male',countmale),rep('female',countfemale))
ID_REF <- c(male_sample,female_sample)
gendata <- data.frame(ID_REF,gender)
genderdata <- merge(description,gendata,by = 'ID_REF')
su <- genderdata %>%group_by(ID,gender)%>%summarise(Counts=n())
genderdata
write.csv(genderdata,file ='GDS5037/genderdata.csv' )

par(mforw=c(1,3))

row <- 1
value <- c(su[row,3]$Counts,su[row+1,3]$Counts)
...
#2.4 group by patients' status,calculate the sample mean and 
#variance of IDENTIFIER FAM174bB in each group
...
library(cdabookfunc)
binom_inference(0,25,0.5,method = 'wald')
